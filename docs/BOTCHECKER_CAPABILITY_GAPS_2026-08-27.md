...
- P0.4 vẫn `PARTIAL`: chưa có production Paper/Bukkit observer-to-signer adapter, trusted key custody/provisioning, effective config/boot/server-instance truth, runner-generated provider evidence, HTTP/report-persistence/release wiring hoặc controlled runtime evidence. Shared local SQLite challenge state, signed observation bytes và offline sealed-bundle re-verification không tự giải quyết các khoảng trống đó. Không được gọi CodeSource/resource hashes, signed `observedAtMs` hay `TARGET_FILE_MATCH_NON_AUTHORITATIVE` là loaded-artifact/fresh-runtime attestation.

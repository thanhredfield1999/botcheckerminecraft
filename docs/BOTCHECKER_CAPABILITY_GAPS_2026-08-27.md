...
- `VERIFIED offline/library-only` ở source/test/build level (2026-08-30): D4d thêm strict schema-v1 resolver cho caller-supplied root policy và policy-aware D4c-c compositor. Caller pin exact policy ID/root-set ID/minimum revision; resolver enforce exact fields, bounded values, half-open active window, explicit retirement, duplicate rejection, stable snapshot/cardinality và deterministic ASCII-order policy hash. Rotation overlap chỉ resolve exact active ID đã pin. Compositor nhận raw policy, resolve trước RPC rồi truyền frozen selected roots vào D4c-c; không nhận forged resolution hoặc expose PEM. Initial review FAIL với một HIGH vì valid PEM có well-shaped SHA pin sai vẫn qua resolver và chạm RPC; RED counterexample đã đóng bằng strict single-PEM parse + exact DER SHA-256 match trước RPC. Policy hash không phải signature/authenticity/rollback/trusted-time proof. Hai capability `library-only`, resolver chỉ do compositor import và compositor chưa có downstream importer. Pre-correction focused `68/68`; exact-current correction `34/34`; exact-current full `547/543/0/4`; typecheck/TypeScript-Java build/diff-check PASS. Claim/import review `deleg_6381866a` và correction review `deleg_1fe99168` đều PASS `0/0/0/0`; HIGH CLOSED. Authoritative roots, authenticated distribution, durable rollback, trusted time/revocation, manual/live/runtime/Paper/deployment vẫn `BLOCKED / NOT VERIFIED`.
- `VERIFIED offline/library-only partial` (2026-08-30): P0.4 thêm compositor tự đọc verified immutable bundle snapshot và independently re-verify đúng một report + một canonical signed-provider observation envelope. Nó bind report reference, run/scenario/capability/target, canonical challenge identity, signed structural window, local trust-store Ed25519 signature và exact non-authoritative candidate observation. Generic writer/verifier thêm aggregate cap `64 MiB`; snapshot chỉ expose deep-frozen canonical base64. Output giữ bundle/report authenticity, report-verdict authentication, challenge issuance, freshness/replay/consume, trusted time, trust-store provenance/rollback, custody, loaded-bytecode, runtime/production/release đều false. Initial implementation review FAIL/HIGH vì Proxy cardinality có thể persist 129 partial files sau count check; hai RED đã đóng bằng reject Proxy trước `length`, exact dense own-index và stable-cardinality snapshot trước I/O. Focused `20/20`, broad `63/63`, full `561/557/0/4`; claim/import review và final correction review `deleg_167e22f7` đều PASS `0/0/0/0`; HIGH `CLOSED`.
- `VERIFIED offline/optional-runner partial` (2026-08-30): `TestRun` có thể nhận một injected factory, snapshot callback một lần, truyền frozen run/scenario/capability/target/artifact request, strict-bind envelope v2 với run/challenge/target/provider/artifacts/candidate observation rồi ghi exact `provider-evidence` + report reference vào cùng create-new bundle. Persistence failure rollback reference trong memory; queued-cancel không gọi provider; provider artifact được persist trước report để collision không để report tham chiếu artifact thiếu. Review tìm thêm callback TOCTOU qua `run.report()`; RED đóng bằng deep-freeze target binding nội bộ qua `await`. Reference ghi explicit `signatureVerified:false`; writer không verify chữ ký, P0.4 verifier vẫn là consumer riêng cần exact trust store. TDD RED→GREEN; focused `25/25`; full `566/562/0/4`; typecheck/TypeScript-Java build/security scan/diff-check PASS.
- `VERIFIED offline/library-only partial` (2026-08-30): generic `SignedProviderObserverSigningPipeline` compose một injected observer với opaque signing adapter trên exact challenge/profile/provider/binding/probe/candidate. Nó recompute challenge identity, reject stale/future time và candidate mismatch trước signing, strict-parse/reassess JVM observation, bound timeout/caller abort và giữ reentrancy lock tới cả observer/downstream signer late callback settle. Initial review `deleg_ddcd5af1` FAIL vì hostile getters/errors có thể expose callback-controlled messages; RED counterexamples đã normalize challenge, observer result, claimed identity và opaque signing input trước side effects, đồng thời snapshot top-level option getters đúng một lần. Capability/import graph vẫn `library-only`; không private-key loader, runner/report/server/Paper/Bukkit importer. Pipeline không consume nonce; verifier riêng mới reject replay. Focused exact-current `59/59`; full `587/583/0/4`; typecheck/TypeScript-Java build/security-claim scan/diff-check PASS. Correction review `deleg_9efbe346` PASS, không còn blocker/high/medium; initial findings CLOSED.
- `VERIFIED offline/library-only partial` (2026-08-30): Java `PaperJvmObservationPort` định nghĩa handoff contract cho plugin Paper tương lai nhưng không import Bukkit/Paper. `capture()` yêu cầu injected primary-thread predicate và snapshot anchor/bounded declared identity/limits/caller-claimed server+boot IDs; `observe()` reject primary thread, snapshot wall clock trên worker ngay trước bounded `JvmArtifactObserver` I/O và atomic-consume context one-shot. Thread/clock supplier errors được normalize. Capability/provenance là `library-only`; không scheduler, plugin lifecycle/cancellation, transport, signer/private key, runner/report/server importer. Focused exact-current `24/24`; full `591/587/0/4`; typecheck/TypeScript-Java build/security-claim scan/diff-check PASS. Independent review `deleg_d1732bb3` PASS, không blocker/high/medium hoặc blocking low.
- `VERIFIED offline/library-only partial` (2026-08-30): Java `PaperJvmObservationClaimBridge` nhận caller-supplied canonical `Claims` + public `PaperJvmObservationPort.Result`, yêu cầu exact observedAt/server/boot equality rồi delegate candidate/hash/non-authoritative posture và canonical UTF-8 cho existing builder. Real-JAR observation bytes parity Node v2; mismatch metadata/hash fail trước output với bounded message. Capability/provenance `library-only`; không Bukkit/Paper, signer/key, nonce store, network, runner/report/server/filesystem/env API. Public result vẫn tự dựng được, nên bridge chỉ bind consistency, không chứng minh port provenance/challenge issuance/freshness/consume/runtime/custody/release. Focused exact-current `28/28`; full `594/590/0/4`; typecheck/TypeScript-Java build/security-claim scan/diff-check PASS. Independent review `deleg_9a6b9d4d` PASS, không blocker/high/medium.
- `VERIFIED offline/library-only partial` (2026-08-30): Java/Node `PaperJvmObservationResultCodec` canonicalize một public port result thành strict JSON v1, cùng cap `16 KiB`; Node yêu cầu exact canonical UTF-8, strict fields, deep-frozen owned snapshot và sanitize hostile getter. Supplementary-Unicode Java→Node parity, malformed UTF-8, whitespace, extra field, oversized và buffer mutation đều có regression. Capability chỉ hiện khi cả hai source halves tồn tại; source/compiled provenance bind và không runtime importer. Không process/socket/HTTP/Paper scheduler/transport/origin authentication/challenge/freshness/nonce/signing/custody/release. Initial review `deleg_951fb9f0` FAIL vì MEDIUM hostile `Uint8Array` Proxy getter leak và LOW caller-controlled Zod diagnostic leak; cả hai được RED→GREEN bằng bounded codec errors. Focused exact-current `29/29`; full `598/594/0/4`; typecheck/TypeScript-Java build/security-claim scan/diff-check PASS. Final correction confirmation `deleg_e606f6b0` PASS, direct hostile/malformed probes PASS, không còn blocker/high/medium/low; findings CLOSED.
- `VERIFIED offline/library-only partial` (2026-08-30): `createPaperJvmObservationByteProvider` biến một injected canonical-byte callback thành existing `SignedProviderObservationProvider`, forward exact frozen challenge/candidate/abort request, strict-parse owned frozen result và normalize callback/AbortSignal Proxy/byte Proxy/malformed errors. Offline adapter→pipeline→opaque signer→verifier composition tạo signature hợp lệ; verifier riêng consume nonce, observation vẫn non-authoritative/release false. Capability chỉ hiện khi adapter + Node codec + observer pipeline + Java codec source đủ; codec chỉ do adapter import, adapter chưa có downstream runtime importer. Focused exact-current `53/53`; full `608/604/0/4`; typecheck/TypeScript-Java build/security-claim scan/diff-check PASS. Independent review `deleg_d19f1213` PASS `0/0/0/0`; targeted adapter `8/8` và composition/capability `3/3` PASS. Không process/socket/HTTP/Paper scheduler/origin authentication/port provenance/challenge freshness/trusted runtime identity/custody/release proof.
- P0.4 vẫn `PARTIAL`: chưa có production Paper/Bukkit observer-to-signer adapter, trusted key custody/provisioning, effective config/boot/server-instance truth, default HTTP factory/release wiring hoặc controlled runtime evidence. Shared local SQLite challenge state, signed observation bytes, optional runner persistence và offline sealed-bundle re-verification không tự giải quyết các khoảng trống đó. Không được gọi CodeSource/resource hashes, signed `observedAtMs` hay `TARGET_FILE_MATCH_NON_AUTHORITATIVE` là loaded-artifact/fresh-runtime attestation.

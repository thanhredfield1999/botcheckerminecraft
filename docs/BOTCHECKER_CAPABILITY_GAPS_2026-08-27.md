...
- `VERIFIED offline partial` (2026-08-28): library-only signed-provider claim verifier đã có strict configured Ed25519 key snapshot, verifier-issued nonce, bounded one-process challenge lifecycle, canonical signature, exact provider/target/artifacts và synchronous one-time consume. Kết quả luôn `artifact-bound`/`releaseEligible:false`; latest pre-commit full gate `374 total / 372 pass / 0 fail / 2 skip` sau credential/timestamp corrections.

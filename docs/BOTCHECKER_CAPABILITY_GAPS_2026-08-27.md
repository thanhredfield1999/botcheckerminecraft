...
- P0.4 vẫn `PARTIAL`: core/assessment chưa có Paper/Bukkit adapter, trusted key custody/signing, shared challenge state, effective config/boot truth, report/bundle/server integration hoặc controlled runtime evidence. Không được gọi CodeSource/resource hashes hay `TARGET_FILE_MATCH_NON_AUTHORITATIVE` là loaded-artifact attestation.
